...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return Math.floor(new Date(lastDate - firstDate) / 1000 / 60 / 60 / 24) + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  return currDate <= endDate && currDate >= startDate;
}
...
function getCountWeekendsInMonth(month, year) {
  let countWeekends = 0;
  const countDaysInMohth = new Date(year, month, 0).getDate();
  for (let i = 1; i <= countDaysInMohth; i += 1) {
    const dayOff = new Date(year, month - 1, i).getDay();
...
function getWeekNumberByDate(date) {
  ...
  const daysSinceStart = (date - yearStart) / DayMS;
...
function isLeapYear(date) {
  ...
  const leap = currYear % 4 === 0 && currYear % 100 !== 0;
  return leap || currYear % 400 === 0;
}
